...
public class SharedPreferences_Utils {
    ...
    public void setDataSharePreferences_From_Chapter(Chapter chapter) {
        ...
        Chapter chapterSave = new Chapter(
                ...
                chapter.getTitle());
        int indexChapter = findIndexChapter(chapterArrayList,chapter.getIdChapter());
        Log.d("21321321",indexChapter+"");
...
//        }
    ...
    private int findIndexChapter(ArrayList<Chapter> chapterArrayList,int idChapter){
        for (int i = 0;i<chapterArrayList.size();i++){
            if (chapterArrayList.get(i).getIdChapter() ==idChapter){
                return i;
            }
        }
        return -1;
    ...
    public ArrayList<Story> get_SharedPreferences_Story_HistoryFragment() {
        Gson gson = new Gson();
        String json = sharedPreferences.getString(SaveStoryWatched, null);
        Type type = new TypeToken<ArrayList<Story>>(){}.getType();
        ArrayList<Story> storyArrayList = gson.fromJson(json, type);
    ...
    public ArrayList<Chapter> getDataSharePreferences_From_ChapterWatched(Story storyss) {
        Gson gson = new Gson();
        String json = sharedPreferences.getString(SaveChapterWatched, null);
        Type type = new TypeToken<ArrayList<Chapter>>(){}.getType();//-> Array_list
        ArrayList<Chapter> list = gson.fromJson(json, type);
        ArrayList<Chapter> chapterList = null;
        if (list!=null) {
            chapterList = chapterOfStory(list, storyss.getId());
        }
        return chapterList;
    }
    ...
    private boolean hasChapterInList(ArrayList<Chapter> chapterArrayListWatched, int idChapter){
        for (int i = 0;i<chapterArrayListWatched.size();i++){
            if (chapterArrayListWatched.get(i).getIdChapter() == idChapter ){
                return true;
            }
        }
        return false;
    ...
    public static float getTextSizeReadStory(){
        float selectedSize = sharedPreferences.getFloat(Size,TextSize.size_default);
        return selectedSize;
    }
    ...
    public static int getScreenTimeOutReadStory(){
        int selectedScreenTimeOut = sharedPreferences.getInt(ScreenTimeOut, TextSize.DEFAULT_SECOUND);
